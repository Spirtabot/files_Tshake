...
database:set("spirta:get_admin_bot:"..bot_id,"ok")
end 
...
database:del("spirta:get_admin_bot:"..bot_id)
end 
local stop2 = nil
if (text and database:get("spirta:get_admin_bot:"..bot_id) and is_sudo(msg) and text == "تفعيل") then
function cb(t1,t2)
for i=1,#t2.members_ do 
if t2.members_[i].status_.ID == "ChatMemberStatusEditor" then 
id = t2.members_[i].user_id_
if id ~= bot_id then 
function zain_ts(y1,y2)
...
database:del('spirta:'..bot_id.."charge:"..msg.chat_id_)
stop2 = "ok"
return "stop"
end
end
