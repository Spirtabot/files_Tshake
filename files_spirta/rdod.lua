...
if not database:get('spirta:'..bot_id..'rep:mute'..msg.chat_id_) then
if text == 'هلو' then
moody = "هلاوات 🌚👋🏻"
...
if  (text and text == 'تفعيل الردود') and is_owner(msg) then
  ...
  send(msg.chat_id_, msg.id_, 1, 'تم تفعيل الردود', 1, 'md')
   database:del('spirta:'..bot_id..'rep:mute'..msg.chat_id_)
  end
  end
  if(text and text == 'تعطيل الردود') and is_owner(msg) then
    if database:get('spirta:'..bot_id..'rep:mute'..msg.chat_id_) then
  send(msg.chat_id_, msg.id_, 1, 'ردود البوت معطله سابقا', 1, 'md')
  else
  send(msg.chat_id_, msg.id_, 1, 'تم تعطيل الردود', 1, 'md')
    database:set('spirta:'..bot_id..'rep:mute'..msg.chat_id_,true)
  end
    end
