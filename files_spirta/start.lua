...
local function zain_spirta(data)
    ...
    if (text and text == 'تعين start' and  tonumber(msg.sender_user_id_) == tonumber(sudo_add) ) then 
      ...
      database:set("zain:addo"..msg.sender_user_id_, "yes")
      return false
    end  
    if text then 
    local zain = database:get("zain:addo"..msg.sender_user_id_)
    if zain == 'yes' then 
      send(msg.chat_id_, msg.id_, 1, "☑️┇تم حفظ كليشه start", 1, 'md')
      database:del("zain:addo"..msg.sender_user_id_)
      database:set("zain:start0"..bot_id, text)
      return false
    end
    end
...
if text == '/start' then 
local start = database:get("zain:start0"..bot_id)
local kkt = ''
if start then 
